...
use crate::keys::DiscoPublicKey;
use crate::Error;

/// A disco message header.
///
/// Outer envelope; not part of the encrypted payload.
#[derive(
    Debug,
    Copy,
    Clone,
...
impl Header {
    /// Magic bytes — "TS" then UTF-8 speech bubble emoji.
    ...
    /// Whether this is a valid disco header (magic check).
    pub const fn is_valid(&self) -> bool {
        matches!(self.magic, Self::MAGIC)
    }

    /// Validate that this header has the right magic number.

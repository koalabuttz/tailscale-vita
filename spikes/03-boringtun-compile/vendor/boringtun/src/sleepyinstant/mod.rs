// vita-patch: was `#![forbid(unsafe_code)]`. Relaxed to `deny` so that
// unix.rs can call `libc::clock_gettime` directly (was via `nix`, which
// hides the unsafe behind a Rust wrapper but is incompatible with the
// Vita target — see Cargo.toml comment).
